...
class Cart extends React.Component  {  
    ...
    drawProductsTable() {
        return this.props.cart.map(product => {
            const currentProd = this.props.products.filter((prod)=>{return prod.id === product})            
            return (
                <tr key={this.props.images[`${product-1}`].id}>
                    ...
                    <td>
                        <div className="large ui buttons center floated">
                            <button className="ui button" onClick={() => this.props.subQuantity(currentProd[0])}>
                                <i className="minus icon"></i>
                            </button>
                            <div className="or"  data-text={this.props.quantity[product]}></div>
                            <button className="ui button" onClick={() => this.props.addQuantity(currentProd[0])}>
                    ...
                    <td>
                        <div className="ui animated button" tabIndex="0" onClick={() => this.props.removeFromCart(currentProd[0])} >
...
export default connect (mapStateToProps, {removeFromCart, addQuantity, subQuantity})(Cart);
